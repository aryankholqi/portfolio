            ...
            <div className="grid gap-3 border-t border-[var(--color-divider)] py-6 md:grid-cols-[130px_1fr_auto] md:gap-8">
              ...
              <div>
                ...
                {item.details.length > 0 && (
                  <ul className="m-0 mt-3 flex list-disc flex-col gap-2 pl-4 text-[13.5px] leading-[1.65] text-[rgba(233,233,237,0.6)]">
                    {item.details.map((d) => (
                      <li key={d}>{d}</li>
                    ))}
                  </ul>
              ...
              <span className="text-[13px] text-[rgba(233,233,237,0.45)] md:text-right">{item.grade}</span>
            </div>
          </Reveal>
        ))}
      </div>
